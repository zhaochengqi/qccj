        ...
        <el-row :style="{ 'min-height': _PageHeight+'px'}">
            <el-form ref="form" :model="form" :rules="rules" label-width="140px">
                ...
                <el-form-item label="杂志图片" required>
                    <el-col :md="12">
                        <QN_upload keyPrefix="image" buttonID="uploadimg" :dragdrop="dragdrop" :filters="filtersImage" max_file_size="500kb" type="drag" @Callback-UploadProgress.self="CB_UploadProgress" @Callback-GetFileUrl.self="CB_GetFileUrl" @Callback-Error.self="CB_Error"></QN_upload>
                ...
                <el-form-item label="杂志文件" required>
                    <el-col :span="12" id="uploadVideo">
                        <QN_upload type="drag" keyPrefix="book" container='uploadVideo' drop_element="uploadVideo" :dragdrop="dragdrops" :filters="filtersBook" max_file_size="200mb" @Callback-UploadProgress.self="CB_UploadProgress_video" @Callback-GetFileUrl.self="CB_GetFileUrl_video" @Callback-Error.self="CB_Error_video"></QN_upload>
                        <div class="el-upload__tip" style="line-height:25px">
                            只能上传pdf文件，且不超过200MB
                        </div>
                    </el-col>
                    <el-col :span="12">
                        ...
                        <span v-else-if="form.pdf_file" class="newsImage">
                            <div id="videosQCCj">
                                <div > {{PDFname}}</div>
                                <span class="el-icon-circle-cross" @click="rmFile"> </span>
                        </div>
                        </span>
                    </el-col>
                </el-form-item>
                <el-form-item label="卷首语" prop="content">
                ...
                <el-form-item>
                    <el-button type="primary" size="large" v-if="authCheck(104)&&authCheck(103)" :loading='loadingbiao' @click="onSubmit">保存杂志信息</el-button>
...
export default {
    ...
    methods: {
        ...
        setData() {
            if (this.form.img == '') {
                this.$message.error('请上传杂志图片！')
                this.loadingbiao = false
                return
            }
            if (this.form.pdf_file == '') {
                this.$message.error('请上传杂志文件！')
                this.loadingbiao = false
                return
            }
            var url
        ...
        getData() {
            ...
            this.axios({
                ...
                if (response.data.status.code !== 1001) {
                    ...
                    this.PDFname = wenjian[wenjian.length - 1];
        ...
        CB_GetFileUrl(file) {
            ...
            this.form.img = file.split('!')[0] + '!230x162'
        },
        ...
        CB_UploadProgress_video(file) {
            this.qn_upload.video_upload_status = ""
        ...
        CB_GetFileUrl_video(file) {
            ...
            this.form.pdf_file = file.split("!")[0];
        },
