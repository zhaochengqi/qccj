    ...
    <el-card class="box-card" v-loading="loading" element-loading-text="拼命加载中">
            ...
            <el-row>
                ...
                <el-col :md="18" :sm="18" class="text-right">
                    <el-button-group>
                        ...
                        <el-button type="danger" v-if="authCheck(93)" icon="delete2" @click="piliangDeleted">批量删除</el-button>
                    </el-button-group>
                </el-col>
            </el-row>
        </div>
        <div v-bind:style="{ 'min-height': _PageHeight+'px'}">
            <el-table :data="tableData" stripe highlight-current-row border style="width: 100%" @selection-change="handleSelectionChange" @cell-click="handleCellClick">
                ...
                <el-table-column prop="link_name" label="友情链接名称" show-overflow-tooltip min-width="200">
                    <template scope="scope">
                        <a :href="scope.row.link_url" target="_Blank" class="text-primary">{{scope.row.link_name}}</a>
                    </template>
                </el-table-column>
                <el-table-column label="链接地址" show-overflow-tooltip min-width="200">
                    <template scope="scope">
                        {{scope.row.link_url}}
                    </template>
                </el-table-column>
                <el-table-column label="操作" width="150" align="center" v-if="authCheck(83)||authCheck(93)">
                    <template scope="scope">
                        <el-button-group>
                            <el-button size="small" type='primary' v-if="authCheck(83)" @click="handleEdit(scope.row.id)">
                                <i class="el-icon-edit"></i> 编辑
                            </el-button>
                            <el-button size="small" type="danger" v-if="authCheck(93)" @click="handleDelete(scope.row.id)">
