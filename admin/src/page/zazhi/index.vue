    ...
    <el-card class="box-card" v-loading="loading" element-loading-text="拼命加载中">
        <div slot="header" class="clearfix">
            <el-row>
                ...
                <el-col :md="18" :sm="18" class="text-right">
                    <el-button-group v-if="authCheck(100)&&authCheck(102)">
                        <el-button type="primary" icon="plus" @click="handleAdd">添加杂志</el-button>
                        <el-button type="danger" icon="delete2" @click="piliangDeleted">批量删除</el-button>
                    </el-button-group>
                    <div v-else>
                        <el-button type="primary" v-if="authCheck(101)" icon="plus" @click="handleAdd">添加杂志</el-button>
                        <el-button type="danger" v-if="authCheck(102)" icon="delete2" @click="piliangDeleted">批量删除</el-button>
                    </div>
                </el-col>
            </el-row>
        </div>
        <div v-bind:style="{ 'min-height': _PageHeight+'px'}">
            <el-table :data="tableData" stripe highlight-current-row border style="width: 100%" @selection-change="handleSelectionChange" :default-sort="{prop: 'add_time', order: 'descending'}" @sort-change="handleSort">
                ...
                <el-table-column prop="title" min-width="350" show-overflow-tooltip label="杂志名称">
                    <template scope="scope">
                        <a :href="url+scope.row.id" target="_Blank" class="text-primary">{{scope.row.title}}</a>
                    </template>
                </el-table-column>
                <el-table-column prop="download_count" min-width="100" label="下载次数" sortable="custom"></el-table-column>
                <el-table-column prop="add_time" label="添加时间" width="150" sortable="custom"></el-table-column>
                <el-table-column label="操作" width="150" align="center" v-if="authCheck(101)||authCheck(102)">
                    <template scope="scope">
                        <el-button-group>
                            <el-button size="small" type='primary' v-if="authCheck(101)" @click="handleEdit(scope.row.id)">
                                <i class="el-icon-edit"></i> 编辑
                            </el-button>
                            <el-button size="small" type="danger" v-if="authCheck(102)" @click="handleDelete(scope.row.id)">
...
export default {
    ...
    data() {
        return {
            ...
            _order: '',
            _sort: '',
            url: process.env.NODE_ENV == "production" ? "http://a.ez-tech.cn/qccj/web/zazhi-read-" : 'http://localhost:8088/zazhi-read-',
            multipleSelection: [],
            tableData: [],
        }
    ...
    methods: {
        ...
        deleteData(id) {
            this.axios({
                method: 'post',
                url: this.API_ROOT + '/api/zazhi/delete',
                data: {
                    id: id
                },
                headers: this.$store.getters.Request_Head
