<template>
    <div>
        ...
        <div :id="container?container:'SelectContainer'" v-if="!type || type == 'default'">
            <el-button :size="button_size?button_size:'small'" type="" :id="buttonID?buttonID:'qn_upload_button'">{{button_text?button_text:'上传文件'}}</el-button>
        </div>
        <!-- 拖动上传 -->
...
export default {
    ...
    methods: {
        ...
        getKey(keyPrefix) {
            let myData = new Date()
                //年
            let nian = myData.getFullYear()
                //月
            let mon = myData.getMonth() + 1
                //日
            let ri = myData.getDate()
                //输出两位月
            if (mon > 0 && mon < 10)
                mon = '0' + mon
                //输出两位日
            ...
            switch (keyPrefix) {
                case 'image':
                    keyPrefix = 'image/' + nian + mon + ri + "/";
                    break;
                case 'video':
                    keyPrefix = 'video/' + nian + mon + ri + "/";
                    break;
                case 'book':
                    keyPrefix = 'book/' + nian + mon + ri + "/";
                    break;
                    //传附件
                case 'file':
                    keyPrefix = 'file/' + nian + mon + ri + "/";
                    break;
            }
            var l = 13;
    ...
    mounted() {
        ...
        Qiniu.uploader({
            ...
            uptoken_url: this.API_ROOT + '/home_api/docking/getqiniutoken', // Ajax请求uptoken的Url，强烈建议设置（服务端提供）
